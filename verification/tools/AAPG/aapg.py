    ...
    for regNo in range(1, 32):
        ...
        if bitwidth == 64:
            asmFile.write("\tld x{0}, {1}(x{2})\n".format(
                regNo, regNo * 8, memoryBaseAddressRegister))
        else:
            asmFile.write("\tlw x{0}, {1}(x{2})\n".format(
                regNo, regNo * 4, memoryBaseAddressRegister))
...
def random_ASM_generator():
    global totalInstructions, percentBaseInstr, perIntegerComputation, \
        perControlTransfer, perLoadStore, perSystemInstr
    global percentSPFloat, percentSPLoadStore, percentSPComputational, \
        percentSPConversionMov, percentSPCompare, PercentSPClassify
    global percentDPFloat, percentDPLoadStore, percentDPComputational, \
        percentDPConversionMov, percentDPCompare, PercentDPClassify
    ...
    while totalInstructions > cv.instructionNumber:
        ...
        if cv.instructionNumber in progressRange:
            if debug != 'nodebug':
                sys.stdout.write('\rRunning AAPG {0}%'.format(
                    progressRange.index(cv.instructionNumber) + 1))
            sys.stdout.flush()
        if extn_prob < percentBaseInstr:
            ...
            elif typ_prob < perIntegerComputation + \
                    perControlTransfer + \
                    perLoadStore:
            ...
            if typ_prob < percentSPLoadStore:
                ...
                opcode = random.choice(op.sp_fp_computational)
                    ...
                    percentSPConversionMov:
                opcode = random.choice(op.sp_fp_conversion_mov)
            elif typ_prob < percentSPLoadStore + percentSPComputational + \
                    percentSPConversionMov + percentSPCompare:
...
''')
    if bitwidth == 64:
        for _ in range(0, initialMemorySize * 1024 // 8):
            asmFile.write('\n\t.dword {0}'.format(
                str(hex(random.getrandbits(64))[:-1])))
    else:
        for _ in range(0, initialMemorySize * 1024 // 4):
            asmFile.write('\n\t.word {0}'.format(
                str(hex(random.getrandbits(32))[:-1])))
...
''')
    asmFile.close()
    if debug != 'nodebug':
        print('\nNumber of loops =', cv.numberOfLoops,
